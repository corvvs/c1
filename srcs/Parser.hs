...
import AST (AST(..), reduceMulDiv, makeMul)
...
parseMulDiv tokens =
    let (factor, rest) = parsePow tokens in
    parseMulDiv' factor rest
...
parsePow tokens =
    let (base, rest) = parseParen tokens
...
parseParen (TokLParen : tokens) = 
    let (subexpr, rest) = parseAddSub tokens in
    case rest of
